...
keys_to_include_in_hash = ["id", "price"]
...
def populate_dict_of_duplicate_docs(hit):
    ...
    hashval = hashlib.md5(combined_key.encode('utf-8-sig')).digest()
...
def scroll_over_all_docs():
    for hit in helpers.scan(es, index='article8'):
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
      if len(array_of_ids) > 1:
        ...
        matching_docs = es.mget(index="article8", doc_type="doc", body={"ids": array_of_ids})
